...
func (s *DpuSideManager) Ping(ctx context.Context, req *lifecyclepb.PingRequest) (*lifecyclepb.PingResponse, error) {
	s.log.V(2).Info("Received heartbeat ping", "from", req.SenderId, "timestamp", req.Timestamp)

	// Record the time we received this ping
	s.setPing(time.Now())

	return &lifecyclepb.PingResponse{
...
func (d *DpuSideManager) Listen() (net.Listener, error) {
	...
	lifecyclepb.RegisterHeartbeatServiceServer(d.server, d)
